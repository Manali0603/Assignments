...
public class jdbcAssignment1 {
    public static void main(String[] args) throws ClassNotFoundException, SQLException {
        ...
        ResultSetMetaData rsmd = emp_details.getMetaData();
...
//                    "   " + emp_details.getInt(4));
        while (emp_details.next()) {
            for (int i = 1; i <= columnsNumber; i++) {
                ...
                String columnValue = emp_details.getString(i);
        ...
        while (empCountDept.next()) {
            ...
            System.out.println(
                    empCountDept.getInt("emp_dept") + "          " +
                            empCountDept.getString("dept_name") + "          " +
                            count_emp);
        }

        //DETAILS OF EMPLOYEES IN A PARTICULAR DEPARTMENT
        System.out.println("\n....EMPLOYEE DETAILS IN A DEPARTMENT....\n");
        ResultSet empDetailsDept = stmt.executeQuery("SELECT e2.emp_id ,e2.emp_name ,e2.emp_city, d2.dept_name,c2.name \n" +
                "     FROM((employeetable e2  \n" +
                "      INNER JOIN departmenttable d2 ON e2.emp_dept = d2.dept_id)\n" +
                "      inner join companytable c2 on e2.emp_company =c2.comid) where e2.emp_dept =923;          ");
        while (empDetailsDept.next()) {
            System.out.println(
                    empDetailsDept.getInt("emp_id") + "          " +
                            empDetailsDept.getString("emp_name") + "          " +
                            empDetailsDept.getString("emp_city") + "          " +
                            empDetailsDept.getString("dept_name") + "          " +
                            empDetailsDept.getString("name"));
        }
        ...
        while (empCountCompany.next()) {
            ...
            System.out.println(
                    empCountCompany.getInt("Company_ID") + "          " +
                            empCountCompany.getString("Company_Name") + "          " +
                            count_emp);
        }
        ...
        ResultSet updated_table = stmt.executeQuery("SELECT e2.emp_id ,e2.emp_name ,e2.emp_city, d2.dept_name,c2.name \n" +
                "     FROM((employeetable e2  \n" +
                "      INNER JOIN departmenttable d2 ON e2.emp_dept = d2.dept_id)\n" +
                "      inner join companytable c2 on e2.emp_company =c2.comid) ;");
        while (updated_table.next()) {
            System.out.println(
                    updated_table.getInt("emp_id") + "          " +
                            updated_table.getString("emp_name") + "          " +
                            updated_table.getString("emp_city") + "          " +
                            updated_table.getString("dept_name") + "          " +
                            updated_table.getString("name"));
        }
